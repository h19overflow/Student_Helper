...
try {
    # Copy configs folder for build context
    if (-not (Test-Path "$ScriptDir/configs")) {
        Copy-Item -Path "$ProjectRoot/backend/configs" -Destination "$ScriptDir/configs" -Recurse -ErrorAction SilentlyContinue
    }
